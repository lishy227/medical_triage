...
class BodyComparisonAgent:
    ...
    def __init__(self, client: Optional[Any] = None, model: Optional[str] = None) -> None:
        # 保留与原调用链兼容的构造签名；当前实现主要采用规则判断。
        self.client = client
        self.model = model

    BODY_ALIASES = {
        "头颅": {"头", "脑", "脑袋", "头部", "头颅", "后脑", "后脑勺", "额头", "太阳穴", "头顶"},
        "眼": {"眼", "眼睛", "眼部"},
        "耳": {"耳", "耳朵", "耳部"},
        "鼻": {"鼻", "鼻子", "鼻部"},
        "口腔": {"口", "嘴", "嘴巴", "牙", "牙齿", "口腔", "口腔内"},
        "喉": {"喉", "喉咙", "咽", "嗓子", "咽喉"},
        "面部": {"脸", "面", "面部", "面颊", "下巴"},
        "足": {"足", "脚", "脚部", "脚踝", "踝", "脚趾"},
        "腿": {"腿", "大腿", "小腿", "膝", "膝盖", "膝关节"},
        ...
        "胸部": {"胸", "胸口", "胸部", "乳房", "心口"},
        "心脏": {"心脏", "心口", "心前区"},
        "上腹部": {"上腹部", "上腹", "胃", "胃部", "心窝", "肚子上面"},
        "下腹部": {"下腹部", "下腹", "小腹", "肚子下面"},
        "腹部": {"腹", "腹部", "肚子", "肚", "肠", "肠道"},
        "双髋部": {"双髋部", "髋部", "髋", "胯", "胯部"},
        "生殖系统": {"生殖系统", "生殖器", "阴部", "私处", "肛门", "会阴"},
        "肩膀": {"肩", "肩膀", "肩部"},
        "胸椎": {"胸椎", "胸背部", "上背"},
        "背部": {"背", "后背", "背部"},
        "腰椎": {"腰椎", "腰椎部"},
    ...
    def _looks_like_clarification(self, current: str, history: str, recent_user_messages: List[str]) -> bool:
    ...
    def _is_strong_new_part_statement(self, current: str, current_parts: Set[str]) -> bool:
        ...
        symptom_markers = ["疼", "痛", "不舒服", "难受", "发麻", "刺痛", "酸胀", "肿", "痒", "硬块"]
        starts_with_part = any(current.startswith(alias) for aliases in self.BODY_ALIASES.values() for alias in aliases)
        has_symptom_marker = any(marker in current for marker in symptom_markers)
    ...
    @staticmethod
    def _no_change(reason: str, detected_body: str = "", confidence: float = 0.0) -> Dict[str, Any]:
        return {
            "is_changed": False,
